...
> **✅ 网关管理端交付（2026-08-16，架构 subagent 设计 + 4 dev team 并行 + 主线程集成）**：**目标**——用户要求 Web 可视化管理「go/zen 套餐切换 + 网关 key（token）定义供其它 agent 使用」，并把所有功能做成管理端。设计文档 `docs/网关管理端设计方案.md`（346 行 11 章）。**核心决策**：① 配置独立 `gateway-config.json`（`~/.local/share/zen-gateway/`，0600，`ZEN_GATEWAY_CONFIG` env 覆盖测试；不放 go-keys.json——职责分离+token 凭据隔离）；② 套餐切换走**重启**（UPSTREAM_BASE/DEFAULT_MODEL 是模块顶层 const 启动固化，热切换要 refactor 协议热路径风险高；切档低频+重启天然一致）；③ 单 token（个人网关 token 只做访问控制，多 token 过度设计）；④ 套餐事实：同一 opencode key 双端点通用，切换只需换 upstreamBase + 默认模型（hy3↔hy3-free）+ 重拉模型表。**Team A（gateway.mjs ~80 行）**：`GATEWAY_CONFIG` 常量 + `ZEN_MODELS_ZEN`（7 免费档）+ `PLANS` 两档表 + `ACTIVE_PLAN/ACTIVE_TOKEN` 模块加载固化（env>文件>默认，旧部署零迁移）+ 上游/默认模型/模型表/探测模型改由 ACTIVE_PLAN 派生 + 鉴权用 ACTIVE_TOKEN + status 补 `plan/authEnabled`；纯函数 `readGatewayConfig/resolvePlan/resolveToken` 导出。测试 **171→191**。**Team B（go-rotate.ts +250 行）**：`GOROTATE_GATEWAY_CONFIG` env + `readGatewayConfig/writeGatewayConfig`（withLockSync+atomicWrite+0600+plan 白名单）+ `maskGatewayToken`/`genToken`（randomBytes 64-hex）+ 新路由 `GET /api/gateway/plans`、`GET /api/gateway/config`（token **掩码返回绝不明文**）、`POST /api/gateway/config`（**只写不重启**，返回 needsRestart，重启由前端显式串 restart——职责分离）；WEB_HTML **主导航 5 区块**（概览/Key 管理/网关管理/统计/设置）+ 套餐切换卡（go/zen 单选→保存→自动串 restart）+ token 管理卡（生成/编辑/清除/复制/掩码 toggle，浏览器 crypto.getRandomValues）+ 设置卡。插件测试 **74→89**。**Team C（CLI +121 行）**：`gateway plan [go|zen]` / `gateway token [gen|clear|set <v>]`（secrets.token_hex(32)，只打印掩码不泄漏明文，写操作持 _with_lock）；`_gateway_config`/`_save_gateway_config`（原子写 0600）；新增 `tests/test-go-rotate-gateway-config.py`（**21 用例/74 断言**）。**Team D（install.sh +11 行）**：install_zen_gateway 生成默认 `gateway-config.json`（`{"plan":"go"}` 0600，幂等不覆盖用户配置）；uninstall 随网关目录删除。**主线程集成**：全测试 **CLI 52 + CLI-gateway 35 + CLI-gateway-config 21 + gateway 191 + 插件 89 全 PASS** + 隔离 md5 铁证；同步副本（go-rotate.ts/gateway.mjs/go-rotate 均 md5 一致）并重启真实网关；真实 18888 默认 go 档（plan:go/26 模型/chat OK）；**隔离 E2E 全链路 PASS**：CLI `plan zen`+`token gen`（64-hex/0600）→ 隔离网关实例 zen 套餐生效（hy3-free/7 模型）+ token 鉴权（无/错 token→401、正确→200）→ Web 主导航 5 区块 + 套餐/token 卡 + 写 config 只写不重启 + 掩码返回 `abcd...7890` + restart 调用。**坑**：有 token 时 `/api/gateway/status` 也要带 Authorization（全路由鉴权，正确行为）；掩码格式是 `前4...后4`（三点省略号非星号）。**遗留（非阻塞）**：真实 8899 管理端待 opencode 重启生效（当前旧插件进程）；套餐切换后需用户手动确认重启（或前端已自动串）；zen 免费档实测一次仅验证鉴权未验证真实模型请求（免费档模型需真实 key 探底，留真机）。
