...
> **✅ 网关使用方式展示 + 按钮双主题重构已交付（2026-08-17，用户需求后派 Team A/B/C 并行 + 主线程集成）**：**Team A（使用方式卡）**：nav-settings 内 gw-config-grid 之后新增 `id="gw-usage-card"`（网关地址 `127.0.0.1:18888` mono 展示 + 3 个 `<details>` 折叠块[curl 直连 / codex `~/.codex/config.toml`[含 web_search="disabled" 坑] / claude code `~/.claude/settings.json`[含 ANTHROPIC_AUTH_TOKEN 劫持坑]，首个默认展开] + 每块复制按钮）；新增 `hydrateUsage()`（`el.textContent = USAGE_TEXT[k]` 零 innerHTML，XSS 红线）+ `copyUsage(k)`/`copyText(txt,okMsg)`（navigator.clipboard → execCommand 降级 + toast）；token 一律 `<ZEN_GATEWAY_TOKEN>` 占位符不读真实值；内容全静态常量零新增后端 API；bootstrap 已接 `hydrateUsage()`。**Team B（按钮双主题）**：`:root`+`html[data-theme="light"]` 各新增 **35 个 `--btn-*` 变量**（中性/primary/danger/success[预留]/ghost/spinner 五族 + hover 变体）；`button` 基础 + primary/danger/ghost/.loading spinner 全部改走变量，新增 `button.success` 规则（预留）；**双主题取值**：深色延续（primary #3b82f6 白字、danger 红 soft 底 #fca5a5）；浅色重设计（primary 深蓝 #2563eb、danger 浅红底 #fdecec+深红字 #b91c1c[旧 #fca5a5 浅色对比仅 2.6:1 → 5.66:1]、中性白底深字、spinner 浅色改深圈）。硬编码色清零（含 `.nav-btn.active` 的 #fff）；class 名 primary/danger/ghost/sm/loading 全保留，31 个既有按钮 HTML 零改动。**Team C（测试+文档）**：新增 4 条断言（按钮类名契约/使用方式锚点/卡片结构与复制函数/双主题变量）→ **107/107（399 expect）**；文档 `docs/测试同步-网关使用方式与按钮主题.md` + `docs/按钮主题设计-管理端.md`。**主线程集成**：bun build ✅ + bun test 107/107（399 expect）✅ + **渲染后内嵌 JS `node --check` ✅**（⚠️ 方法修正：必须 bun 求值 WEB_HTML 提取 script，直接 grep 源码文本会因 TS 模板转义 `\\"` 误报——Team A 三示例 `\\"` 渲染后为 `\"` 合法）；副本已同步 + 8899 重启（PID 63268）；**真实浏览器实测**：使用方式卡 3 折叠块首展开 + curl/codex/claude 三示例逐字节正确（codex 块含 base_url/env_key/web_search 坑注释；`config.toml` 在 summary 标题非代码块内）+ 复制按钮×3 + 占位 token；按钮深色（primary #3b82f6/danger #fca5a5）+ 浅色（primary #2563eb/danger #b91c1c + bodyBg #f4f6f8）双向切换 + `--log` 双主题正确；主题已恢复 dark。提交见 git log。遗留（非阻塞）：浅色 primary 深蓝 #2563eb 与深色 brand-strong 同值属设计选择；success 族预留未接线；复制依赖 Clipboard API（http://127.0.0.1 安全上下文可用）。
